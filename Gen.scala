...
object Config {
    def plugins(
        ...
        plugins += (withMmu match {
            case false =>
                new StaticAddressTranslationPlugin(
                  ioRange = ioRange,
        ...
        if (withEmbeddedJtagTap || withEmbeddedJtagInstruction)
            plugins += new EmbeddedJtagPlugin(
        ...
        plugins += new DispatchPlugin(
          ...
          robIdAt =
              withDistributedRam.toInt // Not having it enabled allows ram block inferation on execution unit context reads
        )
